...
class TestIdleState {
    ...
    void setup() {
        ...
        idleState = new IdleState(mockedHeroModel);
    }
    ...
    void testTransition1() {
        ...
        idleState.transition(mockedHeroController, 1);
    ...
    void testTransition2() {
        //Transition to grounded - moving right
        Mockito.when(mockedJumpBufferTimer.isRunning()).thenReturn(false);
    ...
    void testTransition3() {
        //Transition to grounded - moving left
        Mockito.when(mockedJumpBufferTimer.isRunning()).thenReturn(false);
        Mockito.when(mockedHeroModel.getDirection()).thenReturn(new Vector2d(-1, 0));
    ...
    @Test
    void testGetTag() {
        Assertions.assertEquals(StateTag.IDLE, idleState.getTag());
    }
}
